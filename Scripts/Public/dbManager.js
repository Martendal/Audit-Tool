...
function addAllSubQuestionOption (currentIndex, depth, questionID, domainID, parentQuestionID, parentDomainID, selector)
{
	...
	var result = 0;
	/*console.log ("BEGIN addAllSubQuestionOption");
	...
	for (var i = currentIndex + 1; i < domainsAndQuestions.questions[parentDomainID].length; i++)
	{
		...
		{
			numOfAdd += getTotalNumOfChild (i, parentDomainID) + 1;
			break;
		}
	...
	console.log ("END addAllSubQuestionOption");
	return numOfAdd;
}
...
function printSubDomain ()
{
	...
	for (var i = selectedDomainID; i < domainsAndQuestions.domains.length; i++)
	{
		...
		if (parentDomain != null)
		{
			...
			tbody.setAttribute("class", "custom-tbody opened");
			tbody.setAttribute("style", "background-color: rgba(" + (255) + ","  + (255-(30*i)) + "," + (155) + ", 0.3)");
...
$.get('/getDomains', array = function(c_domainsAndQuestions)
{
	...
	//divDomain = document.getElementById("Main Selector Options");
	...
	/*span  = document.getElementById("Main Selector Selected");
	span.setAttribute("class", "");
	span.appendChild(document.createTextNode(domainsAndQuestions.domains[0].Nom));*/
	...
	for (var i = 0; i < domainsAndQuestions.domains.length; i++) 
	{
		...
		{	
			...
			/*span  = document.createElement("span");
			span.setAttribute("class", "");
			span.appendChild(document.createTextNode(domainsAndQuestions.domains[i].Nom));
			divDomain.appendChild(span);*/
		}
	}
	// Graphical option
